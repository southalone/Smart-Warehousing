...
本模块负责与DEEPSEEK-R1大模型进行交互，提供智能分析功能：
1. 库存分析与优化建议
2. 销售趋势分析与预测
3. 生产规划建议
4. 企业行动建议
...
"""
    ...
    def _summarize_inventory_data(self, inventory_data: List[Dict]) -> str:
        ...
        # 移除unit_price计算，因为Product模型中没有此字段
    ...
    def _call_deepseek_api(self, prompt: str) -> Dict[str, Any]:
        ...
        payload = {
            ...
            "messages": [
                {
                    "role": "user",
                    "content": prompt[:1000]  # 限制prompt长度以减少token消耗
                }
            ],
            "max_tokens": 1500,  # 进一步减少token数量
        ...
        try:
            ...
            response = self.session.post(
                self.base_url,
                json=payload,
                timeout=(2, 4)  # (连接超时2秒, 读取超时4秒) 进一步减少超时时间
            )
            ...
            logger.warning("DEEPSEEK API请求超时，快速失败以避免token消耗")
            raise Exception("API请求超时，已取消以避免token消耗")
